...
```ts twoslash
import { getSolidityStorageSlotBytes } from "@aave-dao/toolbox";
...
```ts twoslash
import { getSolidityStorageSlotAddress } from "@aave-dao/toolbox";
...
```ts twoslash
import { getDynamicArraySlot } from "@aave-dao/toolbox";
...
```ts twoslash
import { getBytesValue } from "@aave-dao/toolbox";
...
```ts twoslash
import { bytes32ToAddress } from "@aave-dao/toolbox";
...
```ts
import { getSolidityStorageSlotAddress, ChainId } from "@aave-dao/toolbox";
...
- **Strings > 31 bytes**: Currently only short strings (≤31 bytes) are supported
